...
function guardarNoticia(&$noticia,$medio,$programa,$epocmail) { 
    ...
    //if ($row==false){
    //    consolelog("No esta ");
     $sqlquery="INSERT INTO news (epocmail, epocnews, medio, programa, noticia, uid, valoracion, uidmail, hora, segundosDelDia, vocabulario, valor, origen, url, ciudad) VALUES ('$epocmail', '$epocnews', '$medio', '$programa', '$noticia','$uidnews', '$valoracion', '$uid', '$hora', '$segundosDelDia', 'no','NULL', 'REC','NULL', 'ROSARIO') " ;
     $results = $dbnews->query($sqlquery);
...
while($res = $results->fetchArray(SQLITE3_ASSOC)){ 
        ...
        if(preg_match('(resumen|sintesis)', $tema) !== 1) continue;
        
        $body= imap_utf8($res['body']);
