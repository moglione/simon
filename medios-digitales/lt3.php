...
foreach ($links as $key => $value) {
    ...
    if( substr_count($value, '-') > 4 ) $salida[]= "http://lt3.com.ar/".$value;
...
for($n=0;$n<$tope;$n++){
    ...
    // se genera la fecha (de hoy)
    // si se scrapean las noticias todos los dias,
    // las noticias que no estan en la base deberian ser del dia actual
    // (dia del scrapping)
    
    
    $timezone  = -4; //(GMT -4:00) Argentina
    $hora=gmdate("h:i", time() + 3600*($timezone+date("I")));
    $fecha=gmdate("h:i d M Y", time() + 3600*($timezone+date("I")));
    ...
    $patron= '#<h1 class="entry-title">(.*?)</h1>#';
    ...
    // se busca el titulo del programa

    $programa="lt3";
    $patron= '#<h1 class="catTitle">(.*?)</h1>#';
    preg_match($patron, $html, $m);
    $programa=($m[1]);
    ...
    // se busca el texto
    $patron='#<div class="entry-content">(.*?)</p>(.*?)</div>#';
    preg_match($patron, $html, $m);
    $texto=(($m[2]));
    ...
    consolelog( $url);
    consolelog( "epoch= ".$epocnews."   (".$fecha.")");
