...
$url = "https://www.pagina12.com.ar/suplementos/rosario12/notas";
...
for($n=0;$n<$tope;$n++){
    ...
    $patron= '#<time datetime="(.*?)" pubdate="pubdate">(.*?)</time>#';
    ...
    // se busca el titulo
    $patron= '#<div class="article-title">(.*?)</div>#';
    preg_match($patron, $html, $m);
    $titulo=($m[1]);
    ...
    $bajada="";
    $patron= '#<div class="article-summary">(.*?)</div>#';
    preg_match($patron, $html, $m);
    ...
    // se busca el texto
    $patron='#<div class="article-text">(.*?)</div>#';
    preg_match($patron, $html, $m);
    $texto=(($m[1]));
    ...
    $uidnews= uniqid ("rosario12_", TRUE);
    $valoracion="ninguna";
    $medio="rosario12";
