...
$url = "http://telefe.com/canal5rosario/";
...
$patron='#<a href="http://telefe.com/canal5rosario/(.*?)"#'; 
...
foreach ($links as $key => $value) {
      $temp[]="http://telefe.com/canal5rosario/".$value;
}
...
for($n=0;$n<$tope;$n++){
    ...
    if(isset($m[3]))$fecha=$m[3]."-".$m[2]."-".$m[1]; 
    $hora="17:00";
    $fecha=$hora." ". $fecha;
    ...
    // se busca el texto
    $patron='#<div class="post-content box">(.*?)<p>(.*?)</div>#';
    preg_match($patron, $html, $m);
    $texto=(($m[2]));
    ...
    $uidnews= uniqid ("canal5_", TRUE);
    $valoracion="ninguna";
    $medio="telefe-canal5.com";
    ...
    $uid=NULL;
    $segundosDelDia=NULL;
    $origen="telefe-canal5.com";
    $programa="telefe-canal5.com";
